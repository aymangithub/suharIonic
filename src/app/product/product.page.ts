...
export class ProductPage {
    ...
    getVariationProducts() {
        this.api.getItem('products/' + this.product.id + '/variations').subscribe(res => {
            this.variations = res;
        }, err => { });
    }
    getRelatedProducts() {
    ...
    goToProduct(product) {
        ...
        let path = this.router.url.substring(0, endIndex);
    ...
    async addToCart() {
        ...
        if (this.selectAdons() && this.setVariations()) {
    ...
    chooseVariation(att, value) {
        ...
        if (this.usedVariationAttributes.every(a => a.selected !== undefined)) {
            this.variations.forEach(variation => {
                const test = new Array(this.usedVariationAttributes.length);
                test.fill(false);
    ...
    buyExternalProduct(id) {
        const options = 'location=no,hidden=yes,toolbar=no,hidespinner=yes';
        const browser = this.iab.create(this.product.external_url, '_blank', options);
        browser.show();
    }
    ...
    getAddons() {
        if (this.product.meta_data) {
            for (const item in this.product.meta_data) {
    ...
    validateform() {
        if (this.addonsList) {
            for (const addon in this.addonsList) {
                for (const item in this.addonsList[addon].fields) {
